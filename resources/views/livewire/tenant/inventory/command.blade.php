<div class="min-h-screen bg-gray-50 dark:bg-gray-900 p-6">
    <div class="max-w-7xl mx-auto">
        ...
        <div class="bg-white dark:bg-gray-800 rounded-lg shadow-sm border border-gray-200 dark:border-gray-700 p-6 mb-6">
            <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
                <div>
                    <h1 class="text-2xl font-bold text-gray-900 dark:text-white">Parámetros Comandas</h1>
                                ...
                                <td class="px-6 py-3 whitespace-nowrap text-center  text-sm font-medium text-gray-900 dark:text-white">
                                    ...
                                    <div class="flex items-center justify-center">
                                        <div class="flex items-center space-x-3">
                                            <!-- Toggle Switch -->
                                            <button type="button"
                                                wire:click="toggleCommandStatus({{ $cd->id }})"
                                                class="relative inline-flex h-4 w-8 items-center rounded-full transition-all duration-200 ease-in-out focus:outline-none focus:ring-2 focus:ring-indigo-500 focus:ring-offset-2 dark:focus:ring-offset-gray-800 hover:shadow-md {{ $cd->status ? 'bg-indigo-600 hover:bg-indigo-700' : 'bg-gray-300 dark:bg-gray-600 hover:bg-gray-400 dark:hover:bg-gray-500' }}"
                                                role="switch"
                                                aria-checked="{{ $cd->status ? 'true' : 'false' }}"
                                                aria-label="Toggle company status">
                                                <span class="inline-block h-3 w-3 transform rounded-full bg-white shadow-sm transition-all duration-200 ease-in-out {{ $cd->status ? 'translate-x-4' : 'translate-x-1' }}"></span>
                                            </button>
                                        </div>
                                    </div>
                                </td>
            ...
            <div class="bg-white dark:bg-gray-800 px-6 py-3 border-t border-gray-200 dark:border-gray-700 rounded-b-lg">
                <div class="flex items-center justify-between">
                    <div class="text-sm text-gray-700 dark:text-gray-300">
                        Mostrando {{ $commands->firstItem() }} a {{ $commands->lastItem() }} de {{ $commands->total() }} resultados
                    </div>
                    <div>
                        {{ $commands->links() }}
                    </div>
                ...
                <form wire:submit.prevent="save" class="p-6 space-y-6">
                    <div class="mb-3">
                        ...
                        <input wire:model="name" type="text" id="name"
                            ...
                            placeholder="Ingrese nombre de la comanda">
                        @error('name') <span class="text-red-600 text-sm">{{ $message }}</span> @enderror
                    </div>
                    <div class="mb-3">
                        <label class="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-2">Ruta de Impresión *</label>
                        <input wire:model="print_path" type="text" id="print_path"
                            class="w-full px-3 py-2 border border-gray-300 dark:border-gray-600 rounded-lg bg-white dark:bg-gray-700 text-gray-900 dark:text-white placeholder-gray-500 dark:placeholder-gray-400 focus:outline-none focus:ring-2 focus:ring-indigo-500 focus:border-indigo-500"
                            placeholder="Ingrese ruta de impresión">
                        @error('print_path') <span class="text-red-600 text-sm">{{ $message }}</span> @enderror
                    </div>
    ...
    <div x-data="{ open: @entangle('confirmingCommandDeletion') }" x-show="open" style="display:none;" class="fixed inset-0 z-50 flex items-center justify-center">
        ...
        <div class="bg-white rounded shadow p-6 z-50 w-full max-w-md">
            ...
            <p class="mb-4">¿Deseas eliminar esta comanda?</p>
            <div class="flex justify-end space-x-2">
                <button type="button" wire:click="cancel" class="px-3 py-1 border rounded">Cancelar</button>
                <button type="button" wire:click="deleteCommand" class="px-3 py-1 bg-red-600 text-white rounded">Eliminar</button>
            </div>
        </div>
    </div>
</div>