<div class="p-6 bg-gray-50 dark:bg-slate-900 min-h-screen transition-colors">
            ...
            <div class="relative group bg-white dark:bg-gray-800 border border-gray-200 dark:border-gray-700 rounded-xl p-5">
                <div class="flex items-start justify-between gap-2">
                    ...
                    <div class="flex gap-1">
                        <button wire:click="moveEntryUp({{ $entry->id }})"
                            @if($loop->first) disabled @endif
                            class="p-1.5 rounded-lg text-gray-400 hover:text-indigo-600 hover:bg-indigo-50 dark:hover:bg-indigo-900/30 disabled:opacity-30 disabled:hover:bg-transparent disabled:hover:text-gray-400"
                            title="Subir">
                            <x-heroicon-o-arrow-up class="w-4 h-4" />
                        </button>
                        <button wire:click="moveEntryDown({{ $entry->id }})"
    ...
    <div wire:key="entry-modal"
         x-data="{
            ...
            handleModalPaste(event) {
                // No interceptar el pegado normal de texto en el título o en el editor de contenido
                const target = event.target;
                if (target.tagName === 'INPUT' || target.tagName === 'TEXTAREA' || target.closest('.ql-editor')) {
                    return;
                }
                ...
                for (const item of items) {
                    if (item.type && item.type.startsWith('image/')) {
                        ...
                        $wire.uploadMultiple('tempFiles', [file]);
                    }
                }
            }
         }"
         x-init="setTimeout(() => initQuill(), 50)"
         x-on:paste.window="handleModalPaste($event)"
            ...
            <div class="space-y-4">
                ...
                <div>
                    <label class="text-sm font-medium text-gray-700 dark:text-gray-300 mb-1 flex items-center">
                        ...
                        <x-field-hint text="Opcional. Adjunta fotos, un PDF o un Excel de apoyo para esta entrada. Puedes elegir varios archivos, arrastrarlos, o pegar (Ctrl+V) una imagen copiada en cualquier parte de esta ventana." />
                    </label>

                    @if($editingEntryId && $existingAttachments->isNotEmpty())
                    <div class="mb-2">
                        <p class="text-xs font-medium text-gray-500 dark:text-gray-400 mb-1">Archivos actuales</p>
                        <div class="grid grid-cols-4 sm:grid-cols-6 gap-2 p-2 bg-gray-100 dark:bg-gray-900 rounded-lg">
                            @foreach($existingAttachments as $att)
                            <div wire:key="existing-att-{{ $att->id }}" class="relative group aspect-square rounded-lg border border-gray-200 dark:border-gray-700 overflow-hidden bg-white dark:bg-gray-800" title="{{ $att->file_name }}">
                                @if($att->isImage())
                                    <img src="{{ route('tenant.instructivos.attachment', $att->id) }}" class="object-cover w-full h-full">
                                @elseif($att->isPdf())
                                    <div class="flex flex-col items-center justify-center p-2 bg-red-50 text-red-600 w-full h-full">
                                        <svg class="w-8 h-8" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="1.5" d="M7 21h10a2 2 0 002-2V9.414a1 1 0 00-.293-.707l-5.414-5.414A1 1 0 0012.586 3H7a2 2 0 00-2 2v14a2 2 0 002 2z"></path></svg>
                                        <span class="text-[9px] font-bold mt-1">PDF</span>
                                ...
                                <button type="button" x-on:click="
                                    Swal.fire({
                                        ...
                                        if (result.isConfirmed) {
                                            $wire.deleteExistingAttachment({{ $att->id }})
                                        }
                                    })
                                " class="absolute top-0 right-0 bg-red-500 text-white p-0.5 m-0.5 rounded opacity-0 group-hover:opacity-100 hover:bg-red-600 transition-opacity">
                    ...
                    <div id="instructivo-dropzone" x-data="{ isDropping: false }"
                        ...
                        <input type="file" x-on:change="if(!window._isHandlingPaste) { @this.uploadMultiple('tempFiles', $event.target.files) }" multiple accept=".png,.jpg,.jpeg,.webp,.pdf,.xlsx,.xls" class="absolute inset-0 w-full h-full opacity-0 cursor-pointer" />
                    </div>
