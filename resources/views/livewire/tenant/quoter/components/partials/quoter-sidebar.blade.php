...
<div class="border-t border-gray-200 dark:border-gray-700 p-6 flex-shrink-0 bg-white dark:bg-gray-800">
    <div class="space-y-4">
        ...
        @else
        @if(!$selectedCustomer)
        <!-- Buscador de cliente en el carrito -->
        <div class="relative w-full">
            <div class="relative">
                ...
                <input
                    ...
                    class="w-full pl-9 pr-3 py-2 text-xs border border-gray-300 dark:border-gray-600 rounded-lg bg-white dark:bg-gray-700 text-gray-900 dark:text-white transition-all outline-none focus:ring-1 focus:ring-indigo-500 animate-soft-glow"
                    @keydown.enter="$wire.searchCustomer()">
            </div>

            <!-- Resultados de búsqueda -->
            @if(!empty($customerResults))
            <div class="absolute z-50 left-0 right-0 bottom-full mb-1 bg-white dark:bg-gray-800 rounded-lg shadow-xl border border-gray-100 dark:border-gray-700 overflow-hidden max-h-60 overflow-y-auto">
        ...
        @else
        <!-- Cliente Seleccionado en el carrito -->
        <div class="bg-green-50 dark:bg-green-900/10 border border-green-200 dark:border-green-800/60 rounded-lg px-3 py-2 text-xs w-full shadow-sm space-y-2">
            ...
            @php
                $hasAddressBlock = !empty($selectedCustomer['address']) || !empty($selectedCustomer['cityName']);
                $hasBranchSelector = !empty($branches) && count($branches) > 1 && !$isEditing;
                $showClearButton = $this->quoterCount > 0;
            @endphp

            @if($hasAddressBlock)
            <div class="flex items-start justify-between gap-2 border-t border-green-200 dark:border-green-800/60 pt-2">
                ...
                @if($showClearButton)
                    @include('livewire.tenant.quoter.components.partials.quoter-clear-button')
                @endif
            </div>
            @endif

            @if($hasBranchSelector)
            <div class="flex items-center gap-1.5 border-t border-green-200 dark:border-green-800/60 pt-2 text-[9px] text-green-700 dark:text-green-400">
                ...
                @if($showClearButton && !$hasAddressBlock)
                    @include('livewire.tenant.quoter.components.partials.quoter-clear-button')
                @endif
            </div>
            @endif

            @if($showClearButton && !$hasAddressBlock && !$hasBranchSelector)
            <div class="flex items-center justify-end border-t border-green-200 dark:border-green-800/60 pt-2">
                @include('livewire.tenant.quoter.components.partials.quoter-clear-button')
            </div>
            @endif
        </div>
