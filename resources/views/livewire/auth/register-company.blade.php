<div>
    <form wire:submit="register" class="space-y-4">
        ...
        <div>
            ...
            <select wire:model.live="merchant_type_id" id="merchant_type_id" class="block mt-1 w-full border-gray-300 focus:border-indigo-500 focus:ring-indigo-500 rounded-md shadow-sm" required>
        ...
        <!-- Plan de Servicio -->
        @if($merchant_type_id && $plains->count() > 0)
        <div>
            <label for="plain_id" class="block text-sm font-medium text-gray-700">
                Plan de servicio <span class="text-red-500">*</span>
            </label>
            <select wire:model="plain_id" id="plain_id" class="block mt-1 w-full border-gray-300 focus:border-indigo-500 focus:ring-indigo-500 rounded-md shadow-sm" required>
                <option value="">Selecciona un plan</option>
                @foreach($plains as $plain)
                    <option value="{{ $plain->id }}">
                        {{ $plain->name }}
            ...
            <x-input-error :messages="$errors->get('plain_id')" class="mt-2" />
            <p class="mt-1 text-sm text-gray-500">Selecciona el plan que mejor se adapte a tus necesidades</p>
        </div>
        @elseif($merchant_type_id && $plains->count() === 0)
            ...
            <div class="flex">
                ...
                <div class="ml-3">
                    ...
                    <div class="mt-2 text-sm text-yellow-700">
                        <p>No se encontraron planes activos para el tipo de negocio seleccionado. Por favor, contacta al administrador.</p>
                    </div>
                </div>
            </div>
        </div>
