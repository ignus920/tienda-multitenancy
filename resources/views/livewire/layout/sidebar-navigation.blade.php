...
<div class="flex h-full flex-col" :class="sidebarCollapsed ? 'overflow-visible' : 'overflow-y-auto'">
    ...
    <div class="flex shrink-0 items-center justify-center border-b border-gray-200 dark:border-gray-700"
        ...
        {{-- Menú expandido: logo completo (círculo + texto) --}}
        <img x-show="!sidebarCollapsed" class="h-12 w-full max-w-[220px] object-contain" src="{{ asset('images/logofervi.png') }}" alt="Fervicom Iluminación LED">
    </div>
    ...
    <nav class="flex flex-1 flex-col p-4 space-y-1">
        ...
        <!-- Proyectos -->
        @if(Auth::user()?->profile_id != 17 && Auth::user()?->profile_id != 18 && PermissionHelper::userCan('Proyectos', 'show'))
        ...
        <div x-data="{
            ...
            <!-- Botón principal -->
            <div class="group flex items-center rounded-lg px-3 py-2.5 text-sm font-medium transition-all duration-200 {{ request()->routeIs('tenant.task-planner.my-tasks') || request()->routeIs('tenant.task-planner') ? 'text-indigo-600 dark:text-indigo-400 bg-indigo-50/50 dark:bg-indigo-900/20' : 'text-gray-700 dark:text-gray-300 hover:bg-gray-50 dark:hover:bg-gray-800 hover:text-indigo-600 dark:hover:text-indigo-400' }} cursor-pointer"
        ...
        <!-- Solicitudes -->
        @if (Auth::user()?->profile_id != 18 && PermissionHelper::userCan('Solicitudes', 'show'))
        ...
        <!-- Instructivos -->
        @if (PermissionHelper::isSuperAdmin() || PermissionHelper::userCan('Instructivos', 'show'))
        <a href="{{ route('tenant.instructivos') }}" wire:navigate
            class="group relative flex items-center rounded-lg px-3 py-2.5 text-sm font-medium transition-all duration-200 {{ request()->routeIs('tenant.instructivos*') ? 'bg-indigo-50 dark:bg-indigo-900/50 text-indigo-700 dark:text-indigo-300 border-r-2 border-indigo-500' : 'text-gray-700 dark:text-gray-300 hover:bg-gray-50 dark:hover:bg-gray-800 hover:text-indigo-600 dark:hover:text-indigo-400' }}"
            :class="sidebarCollapsed ? 'justify-center' : 'justify-start'"
            x-data="{ tooltip: false }" @mouseenter="tooltip = sidebarCollapsed" @mouseleave="tooltip = false">
