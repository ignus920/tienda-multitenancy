<div x-data="{ 
    ...
    <div class="flex items-end space-x-2">
        <div class="flex-1">
            @if ($showLabel)
                <label for="house_{{ $name }}" class="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-2 flex items-center select-none">
                    {{ $label }}
                    ...
                    <div x-data="{ show: false }" class="relative inline-block ml-1.5">
                        ...
                        <div x-show="show" x-cloak x-transition class="absolute bottom-full left-1/2 transform -translate-x-1/2 mb-2 w-48 p-2 bg-gray-900 text-white text-[10px] rounded-lg shadow-xl z-50 text-center font-normal leading-normal normal-case">
                            Casa o fabricante del producto.
                        </div>
                    </div>
                </label>        
            @endif
        
            <input type="hidden" name="{{ $name }}" wire:model="houseId">
            ...
            <button x-ref="button" x-on:click="toggle()" :aria-expanded="open" :aria-controls="$id('dropdown-button')"
                ...
                <span class="block truncate">
                    {{ $this->selectedHouseName ?? $placeholder }}
                </span>
            </button>
            <div x-ref="panel" x-show="open" x-transition.origin.top.left x-on:click.outside="close($refs.button)"
                ...
                <ul
                    ...
                    <li class="text-gray-900 dark:text-gray-100 relative cursor-default select-none py-2 pl-3 pr-9 hover:bg-indigo-600 hover:text-white dark:hover:bg-indigo-500"
                        role="option" x-on:click="$wire.selectHouse(''); close($refs.button)">
                        <span class="font-normal block truncate">{{ $placeholder }}</span>
                    </li>
                    @forelse($this->houses as $house)
                    <li wire:key="house-{{ $house->id }}"
                        ...
                        role="option" x-on:click="$wire.selectHouse({{ $house->id }}); close($refs.button)">

                        <span class="font-normal block truncate {{ $houseId == $house->id ? 'font-semibold' : '' }}">
                            {{ $house->name }}
                        </span>
